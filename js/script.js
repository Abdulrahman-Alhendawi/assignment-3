...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // QUOTE API (Quotable, no key required)
    ...
    fetch('https://api.quotable.io/random')
      ...
      .then(data => {
        if (data.content && data.author) {
          quoteInfo.innerHTML = `“${data.content}”<br><span style=\"font-size:0.9em;\">— ${data.author}</span>`;
        } else {
          quoteInfo.textContent = 'Quote unavailable.';
        }
      })
      .catch(() => {
        quoteInfo.textContent = 'Unable to fetch quote. Please try again later.';
      });
  }
});
